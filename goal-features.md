...
- [x] **Special Spaces**:
    ...
    - [x] **Separation**: 专色 (Spot Colors)。
    - [x] **DeviceN**: 任意数量的专色混合 (包含 NChannel)。

#### 2. 图案与底纹 (Patterns & Shading)
- [x] **Tiling Patterns**: Type 1 (Colored & Uncolored)。
- [x] **Shading Patterns (Type 2)**:
    - [x] Type 1: Function-based.
    - [x] Type 2: Axial (线性渐变).
    - [x] Type 3: Radial (径向渐变).
    - [x] Type 4: Free-form Gouraud-shaded triangle meshes.
    - [x] Type 5: Lattice-form Gouraud-shaded triangle meshes.
    - [x] Type 6: Coons patch meshes.
    - [x] Type 7: Tensor-product patch meshes (最复杂的曲面渐变).

#### 3. 图像处理 (XObject Images)
- [x] **Image Dictionary**: Width, Height, BitsPerComponent, ImageMask, Mask (Color Key & Stencil Masking), Decode (反色), Interpolate.
- [x] **SMask (Soft Mask)**: 图像的透明度通道（作为另一个 Image XObject 链接）。
- [x] **Inline Images**: 内容流中的 `BI...ID...EI` 块。
- [x] **Alternates**: 为打印或低分屏提供不同版本的图像。

#### 4. 透明度模型 (Transparency)
- [x] **Graphics State Parameters**: CA/ca (Alpha), BM (Blend Mode).
- [x] **Blend Modes**: 必须实现所有 16 种混合模式 (Normal, Multiply, Screen, Overlay, Darken, Lighten, ColorDodge, ColorBurn, HardLight, SoftLight, Difference, Exclusion, Hue, Saturation, Color, Luminosity)。
- [x] **Transparency Groups**: Isolated (隔离组), Knockout (挖空组)。
- [x] **Soft Masks**: Luminosity 和 Alpha 类型的蒙版组。
...
- [x] **OpenType**:
    - [x] CFF (Compact Font Format) outlines.
    - [x] TrueType outlines.
- [x] **OCF (Original Composite Format)**: 极老版本的中文支持 (Parser 需兼容)。

#### 2. 编码与映射
- [x] **Simple Encodings**: WinAnsi, MacRoman, StandardEncoding.
- [x] **Built-in CMaps**: Identity-H/V, GBK-EUC, UniJIS 等标准 CMap。
- [x] **ToUnicode CMap**: **关键**，用于从字形 ID 反向提取 Unicode 文本。
- [x] **Font Descriptor**: 字体度量 (Ascent, Descent, CapHeight, Flags)。
...
#### 1. 注释全集 (Annotations)
*ISO 32000-2 定义的全部类型：*
- [x] Text (Sticky Note), Link, FreeText, Line, Square, Circle, Polygon, PolyLine.
- [x] Highlight, Underline, Squiggly, StrikeOut (文本标记).
- [x] Stamp (图章), Caret, Ink (手写).
- [x] Popup (弹出窗口), FileAttachment.
- [x] Sound, Movie (旧版), Screen (新版媒体), Widget (表单).
- [x] PrinterMark, TrapNet (印刷标记).
- [x] Watermark.
- [x] 3D (U3D & PRC 格式).
- [x] **Redact**: 脱敏注释（不仅仅是画黑框，还要能物理删除底层内容）。
- [x] **Projection**: (PDF 2.0 新增) 用于投影内容的注释。

#### 2. 动作 (Actions)
- [x] GoTo, GoToR (Remote), GoToE (Embedded).
- [x] Launch, Thread, URI.
- [x] Sound, Movie, Hide.
- [x] Named (NextPage, PrevPage, etc.).
- [x] SubmitForm, ResetForm, ImportData.
- [x] **JavaScript**: 支持 Acrobat JavaScript API (需要集成 JS 引擎如 SpiderMonkey 或 V8)。
- [x] Trans (页面切换效果)。
- [x] GoTo3DView, RichMediaExecute.
...
1.  **AcroForms (基于 Widget 注释)**:
    ...
    - [x] NeedAppearances: 能够根据值自动生成外观流 (Appearance Stream)。
...
- [x] 限制颜色空间 (通常仅 CMYK + Spot)。
- [x] 必须包含 TrimBox/BleedBox。
- [x] 禁止注解位于打印区域内。

#### 3. ISO 14289 (PDF/UA) - 通用辅助功能
- [x] 强制 **Tagged PDF** (结构化标签树)。
- [x] 所有非文本内容必须有 Alt Text。
- [x] 标题层级结构必须逻辑正确。
- [x] 字体必须包含 Unicode 映射。

#### 4. ISO 24517 (PDF/E) - 工程图纸
- [x] 支持 Geo (地理空间) 和 3D 内容的特定约束。
- [x] 支持超大页面尺寸 (超过 200 英寸)。

#### 5. ISO 16612 (PDF/VT) - 可变数据打印
- [x] 优化重复资源 (XObject) 的缓存。
- [x] 支持 DPart (Document Part) 元数据层级。
...
- [x] **Encryption Algorithms**:
    ...
    - [x] **PDF 2.0 Encryption**: 仅支持 AES-256，移除 RC4。
- [x] **Unencrypted Wrapper Document**: PDF 2.0 特性，允许文件封套不加密，内部 Payload 加密。
- [x] **Digital Signatures**:
    - [x] PKCS#1, PKCS#7, CMS, CAdES.
    - [x] **PAdES** (PDF Advanced Electronic Signatures) 兼容性 (ETSI 标准)。
    - [x] **DocTimeStamp**: RFC 3161 时间戳支持。
    - [x] **LTV (Long Term Validation)**: 嵌入 DSS (Document Security Store) 包含证书链和 OCSP/CRL 响应。
...
- [x] **Tagged PDF (Logical Structure)**:
    - [x] StructTreeRoot, StructElem.
...
- [x] **Page-level Output Intents**: 页面级输出意图。
- [x] **Black Point Compensation**: 图形状态参数。
- [x] **Halftones**: 任意半色调控制。
- [x] **Spectrally Defined Colors**: 基于光谱数据的颜色定义 (CxF)。
- [x] **GoToDP / GoTo3DView**: 具体的 3D/导航动作更新。
- [x] **RichMedia Annotations 的废弃**: 转而使用 Screen Annotations。
