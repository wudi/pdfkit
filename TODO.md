...
- [ ] Full JPX/JBIG2 native decoding: evaluate and integrate a maintained Go or cgo-backed decoder (e.g., OpenJPEG or libjbig2dec wrappers) to avoid external tool dependency and handle alpha, palettes, and symbol dictionaries with deterministic limits/tests. Status: In progress (JPXDecode now uses libopenjp2 via the optional `openjpeg` build tag with native tests; JBIG2 continues to use the libjbig2dec path, and remaining work tracks broader limit enforcement/palette coverage).
