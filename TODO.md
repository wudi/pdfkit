...
- [x] Logging hooks: surface observability.Logger in writer Config and Pipeline to log start/completion/error events for parse/write phases per design.md observability section. Status: Done (writer Config gains Logger, logs start/finish/errors; pipeline WithLogger added and emits start/finish/error logs; tests cover both success and error paths).
